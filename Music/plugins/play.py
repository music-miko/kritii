...
async def play_music(_, message: Message, context: dict):
    ...
    if url:
        ...
        # ✅ FIX: handle no result case
        if not result:
            return await hell.edit("No results found for this link.")
    ...
    # ✅ safer: avoid IndexError when user sends only /play
    parts = message.text.split(" ", 1)
    if len(parts) < 2 or not parts[1].strip():
        return await hell.edit("Please provide a song name or YouTube link after the command.")
    ...
    # ✅ FIX: handle no result case
    if not result:
        return await hell.edit("No results found for your query.")
